...
export const authenticateUser = async (req, res, tempChange) => {
  try {
    let updatedUsername, updatedPassword;
    if(tempChange) {    
      const { changeUsername, changePassword } = tempChange()
      updatedUsername = changeUsername;
      updatedPassword = changePassword;
    }
    ...
    const AuthURLComplete = `${AuthURL}lt=${LoginToken}&username=${updatedUsername || username}&password=${updatedPassword || password}&service=${CSRFURL}`;
    ...
    const defaultSecContext = await getDefaultSecurityContext(headersForFutureRequests);
