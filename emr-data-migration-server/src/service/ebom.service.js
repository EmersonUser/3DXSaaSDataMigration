...
export const searchProduct = async (req, item) => {
    try {
      ...
      const parentSearchUrl = `https://oi000186152-us1-space.3dexperience.3ds.com/enovia/resources/v1/modeler/dseng/dseng:EngItem/search?$searchStr=${encodeURIComponent(
        item["Parent Name"])}+revision=${item["Parent Rev"]}`;

      const childSearchUrl = `https://oi000186152-us1-space.3dexperience.3ds.com/enovia/resources/v1/modeler/dseng/dseng:EngItem/search?$searchStr=${encodeURIComponent(
...
export const extractChildConnections = async (req, parentId, childId) => {
    try {
        ...
        const headers = createHeaders(cookie, csrf, "VPLMProjectLeader.0000000001.Micro Motion");
        const url = `https://oi000186152-us1-space.3dexperience.3ds.com/enovia/resources/v1/modeler/dseng/dseng:EngItem/${parentId}/dseng:EngInstance?$mask=dsmveng:EngInstanceMask.Details`;
        ...
        const isChildConnected = childConnections.member.some(
            (child) => child.referencedObject?.id === childId
        );
...
// Service for creating EBOM connection
export const createEngInstance = async (req, parentId, childId) => {
    try {
      const cookie = req?.Cookie;
      const csrf = req?.ENO_CSRF_TOKEN;
      ...
      const url = `https://oi000186152-us1-space.3dexperience.3ds.com/enovia/resources/v1/modeler/dseng/dseng:EngItem/${parentId}/dseng:EngInstance`;
      ...
      if (response) {
        return { status: "Engineering instance created successfully", data: response };
      } else {
        return { status: "Failed to create engineering instance", error: response };
      }
    } catch (error) {
      console.error("Error creating engineering instance:", error);
      return { status: "An error occurred while creating engineering instance", error: error.message };
    }
};

export const updateEngInstance = async (req, parentId, instanceData, updateData) => {
    try {
      const cookie = req?.Cookie;
      const csrf = req?.ENO_CSRF_TOKEN;
      ...
      const headers = createHeaders(cookie, csrf, "VPLMAdmin.Company%20Name.Default");
      const url = `https://oi000186152-us1-space.3dexperience.3ds.com/enovia/resources/v1/modeler/dseng/dseng:EngItem/${parentId}/dseng:EngInstance/${instanceData.id}?$fields=dsmveno:CustomerAttributes`;
      ...
      if (response) {
        return { status: "Engineering instance updated successfully", data: response };
      } else {
        return { status: "Failed to update engineering instance" };
      }
    } catch (error) {
      console.error("Error updating engineering instance:", error);
      return { status: "Error while updating engineering instance" };
    }
};
